...
export const FloatingDateField = forwardRef<HTMLInputElement, DateFieldProps>(
  ({ value, label, onChange, fromYear, toYear, disableFuture, forcelightmode = false, ...rest }, ref) => {
    ...
    return (
      ...
      >
        <div className={cn("relative", lightModeClasses)}>
          <Input
            ref={ref}
            id={rest.name}
            name={rest.name}
          ...
          <PopoverTrigger disabled={rest.disabled}
            ...
            className={cn("size-7 p-1.5 absolute right-1 top-1/2 -translate-y-1/2 cursor-pointer flex justify-center items-center text-slate-400 hover:text-slate-600 transition-colors",
        ...
        <PopoverContent className={cn("p-3", lightModeContent)} align="end" onOpenAutoFocus={(e) => e.preventDefault()}>
